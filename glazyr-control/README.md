...
## AWS Deployment (Lambda Function URL)

### Prerequisites

- AWS CLI configured
- AWS SAM CLI installed (`sam --version`)
- IAM permissions: Lambda, Secrets Manager, CloudFormation
...
# 1. Create OpenAI key secret (if not using env var)
aws secretsmanager create-secret `
  --name glazyr-control/openai-api-key `
  --secret-string "sk-..." `
  --region us-east-1

# Copy the ARN from output (e.g., arn:aws:secretsmanager:us-east-1:...)

# 2. Deploy SAM stack
powershell -NoProfile -ExecutionPolicy Bypass -File .\deploy-sam.ps1 `
  -Region us-east-1 `
  -Stage dev `
  -OpenAIKeySecretArn "arn:aws:secretsmanager:us-east-1:123456789012:secret:glazyr-control/openai-api-key-XXXXX" `
  -ApiKey "your-client-access-key" `
  -MaxPayloadSizeMB 5 `
  -AllowedDomains "example.com,api.example.com"

# The script prints the Function URL - use this as GLAZYR_CONTROL_RUNTIME_URL
```
