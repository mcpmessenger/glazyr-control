...
exports.GooglePlacesConnector = void 0;
class GooglePlacesConnector {
    ...
    async authorize(context) {
        ...
        const apiKey = this.config.apiKey || (context.apiKeyRef ? await this.resolveApiKey(context.apiKeyRef) : null);
        return {
    ...
    async execute(step, constraints) {
        ...
        try {
            ...
            switch (action) {
                case 'places_search':
                    output = await this.placesSearch(step.input, constraints);
                    break;
                case 'place_details':
                    output = await this.placeDetails(step.input, constraints);
                    break;
                case 'place_photos':
                    output = await this.placePhotos(step.input, constraints);
                    break;
                case 'place_hours':
                    output = await this.placeHours(step.input, constraints);
                    break;
                case 'place_reviews':
                    output = await this.placeReviews(step.input, constraints);
                    break;
                default:
                    return {
                        status: 'error',
    ...
    async placesSearch(input, constraints) {
        const apiKey = await this.getApiKey();
        if (!apiKey)
        ...
        const results = (data.places || []).slice(0, maxResults).map((place) => ({
            place_id: place.id,
    ...
    async placeDetails(input, constraints) {
        const apiKey = await this.getApiKey();
        if (!apiKey)
            throw new Error('API key not available');
    ...
    async placePhotos(input, constraints) {
        const apiKey = await this.getApiKey();
        if (!apiKey)
            throw new Error('API key not available');
    ...
    async placeHours(input, constraints) {
    ...
    async placeReviews(input, constraints) {
...
exports.GooglePlacesConnector = GooglePlacesConnector;
