...
const { performOcr, performAnalyze } = require("./google-vision")
...
exports.handler = async (event) => {
  ...
  try {
    ...
    //   "features": { "ocr": true, "labels": true, "objects": true }
    // }
    //
    // Response (200):
    // {
    //   "text": "...",
    //   "caption": "Objects: ... · Labels: ... · Text detected",
    //   "labels": [{ "description": "...", "score": 0.98 }],
    //   "objects": [{ "name": "...", "score": 0.92, "boundingPoly": {...} }],
    //   "labelAnnotations": [...],
    //   "localizedObjectAnnotations": [...]
    // }
    if (method === "POST" && path.endsWith("/runtime/vision/analyze")) {
      const body = parseJsonBody(event)
      if (body === "__invalid_json__") return json(400, { error: "Invalid JSON" })
      ...
      try {
        const res = await performAnalyze(imageDataUrl, { features })
        return json(200, res)
      } catch (e) {
        const msg = String(e?.message || e)
        const lower = msg.toLowerCase()
